...
class BlurbFeatures:
  ...
  ''' Computes and returns the TD-IDF matrix, where each row is a document, and each column is a word in the corpus.
  inputs:
  ...
  '''
  def compute_training_tfidf_matrix(self, training_blurbs, min_df=0.025, max_df=0.25):
    self.count_vectorizer = CountVectorizer(ngram_range=(1,2), min_df=min_df, max_df=max_df)
    print('dim training blurbs, before cleaning', training_blurbs.shape)
    cleaned_blurbs = training_blurbs.map(BlurbFeatures._clean_text)
    print('dim cleaned_blurbs ', cleaned_blurbs.shape)
  ...
  '''
  def compute_training_svd_df(self, training_tfidf_matrix = None, n_components = 30):
    ...
    self.svd_transformer = TruncatedSVD(n_components=n_components)
    ...
    col_names = ["svd_" + str(i) for i in range(1, n_components + 1)]
  ...
  def compute_testing_svd_df(self, testing_dfidf_matrix, n_components = 30):
  ...
  def compute_testing_svd_df_from_blurbs(self, testing_blurbs, n_components=30):
    testing_tdidf_matrix = self.compute_testing_tfidf_matrix(testing_blurbs)
    return self.compute_testing_svd_df(testing_tdidf_matrix, n_components=n_components)
